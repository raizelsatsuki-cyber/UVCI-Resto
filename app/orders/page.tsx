...
import {
  Loader2, Package, Clock, CheckCircle, BellRing, ShoppingBag,
  ArrowLeft, QrCode, ChevronDown, ChevronUp, XCircle, AlertTriangle,
} from 'lucide-react';
...
const STATUS_MAP: Record<string, { label: string; className: string; icon: React.ReactNode }> = {
  ...
  pending:         { label: 'En attente',              icon: <Clock size={13} />,       className: 'bg-orange-100 text-orange-700 border-orange-200' },
  ...
  cancelled:       { label: 'Annulée',                 icon: <XCircle size={13} />,    className: 'bg-red-100 text-red-500 border-red-200' },
};
...
/** Statuts où l'utilisateur peut encore annuler */
const CANCELLABLE_BY_USER = new Set(['pending', 'pending_payment']);
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full">
        <div className="flex flex-col items-center text-center gap-4">
          ...
          <div>
            <h3 className="text-lg font-extrabold text-gray-800 mb-1">Annuler la commande ?</h3>
            <p className="text-sm text-gray-500">Cette action est irréversible. Votre commande sera annulée et le stock restitué.</p>
          </div>
          <div className="flex gap-3 w-full">
            ...
            <button onClick={onConfirm} disabled={loading}
              className="flex-1 py-3 rounded-xl bg-red-500 text-white font-bold hover:bg-red-600 transition text-sm flex items-center justify-center gap-2 disabled:opacity-60">
...
export default function ClientOrdersPage() {
  ...
  const handleCancel = async () => {
    ...
    setCancelling(true);
    try {
      await cancelOrder(cancelTarget, false);
        ...
        <div className="space-y-4">
          {orders.map(order => (
            <Card3D key={order.id} className="p-5">
              ...
              {CANCELLABLE_BY_USER.has(order.status) && (
                <button
                  onClick={() => setCancelTarget(order.id)}
                  className="w-full flex items-center justify-center gap-2 py-2 mb-3 bg-red-50 border border-red-200 rounded-xl text-red-500 font-bold text-sm hover:bg-red-100 transition"
                >
